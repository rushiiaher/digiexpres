...
const AdminDashboard = () => {
  ...
  const handleDeleteBlog = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this post?')) {
      try { await deleteDoc(doc(db, 'blogs', id)); } catch (error) { console.error("Error deleting blog", error); }
    }
  };

  const handleSeedBlogs = async () => {
    if (window.confirm('Load initial mock data into Firestore? This will add 2 dummy posts.')) {
      const dummyPosts = [
        { title: "The Future of AI in Enterprise Software", excerpt: "Discover how artificial intelligence is reshaping the landscape of enterprise software...", content: "Full content here", date: "2024-03-15", author: "Sarah Johnson", category: "AI & Machine Learning", image: "https://images.unsplash.com/photo-1677442136019-21780ecad995" },
        { title: "Securing Cloud Infrastructure in 2024", excerpt: "Essential security measures every organization needs to implement for their cloud infrastructure.", content: "Full content here", date: "2024-03-10", author: "Michael Chen", category: "Cybersecurity", image: "https://images.unsplash.com/photo-1544197150-b99a580bb7a8" }
      ];
  ...
  const handleAddCategory = async (e: React.FormEvent) => {
    ...
    try { await addDoc(collection(db, 'blogCategories'), { name: newCategory.trim() }); setNewCategory(''); }
    catch { alert('Error adding category'); }
  };

  const handleDeleteCategory = async (id: string) => {
    if (window.confirm("Delete this category?")) await deleteDoc(doc(db, 'blogCategories', id));
  };

  // ─── Contact Handlers ─────────────────────────────────────────────────────
  const markMessageRead = async (id: string) => {
    try { await updateDoc(doc(db, 'contacts', id), { status: 'read' }); } catch (error) { console.error("Error marking read", error); }
  };
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="bg-white border-b border-slate-200 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-3 flex items-center justify-between">
          <span className="font-bold text-slate-800 text-base sm:text-lg">Digiexpres Admin</span>
          <button
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 lg:flex lg:gap-8 lg:items-start">

        {/* ── Sidebar / Tab Navigation ── */}
        <aside className="mb-4 lg:mb-0 lg:w-56 lg:flex-shrink-0">

          {/* Mobile: horizontal scrollable pill tabs */}
          <div className="flex lg:hidden bg-white border border-slate-200 rounded-lg p-1 gap-1 overflow-x-auto">
            <button
              onClick={() => setActiveTab('blogs')}
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-md font-medium text-sm transition-colors ${activeTab === 'blogs' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              <FileText className="h-4 w-4" /> Blogs
            </button>
            <button
              onClick={() => setActiveTab('categories')}
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-md font-medium text-sm transition-colors ${activeTab === 'categories' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              <Tags className="h-4 w-4" /> Categories
            </button>
            <button
              onClick={() => setActiveTab('contacts')}
              className={`flex-shrink-0 flex items-center gap-2 px-4 py-2 rounded-md font-medium text-sm transition-colors ${activeTab === 'contacts' ? 'bg-blue-600 text-white' : 'text-slate-600 hover:bg-slate-100'}`}
            >
              <Mail className="h-4 w-4" /> Inquiries
              {newCount > 0 && (
                <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold ${activeTab === 'contacts' ? 'bg-white text-blue-600' : 'bg-blue-100 text-blue-600'}`}>
                  {newCount}
                </span>
              )}
            </button>
          ...
          <div className="hidden lg:flex flex-col space-y-2">
            ...
            >
              <div className="flex items-center gap-3"><Mail className="h-5 w-5" /> Inquiries</div>
              {newCount > 0 && (
        ...
        {/* ── Main Content Area ── */}
        <main className="flex-1 bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden min-h-[500px]">
          ...
          {activeTab === 'blogs' && (
            <div className="p-4 sm:p-6">
              ...
              {showForm ? (
                <div className="bg-slate-50 border border-slate-200 p-4 sm:p-6 rounded-lg mb-8">
                  ...
                  <form onSubmit={handleBlogSubmit} className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <input type="text" placeholder="Title" value={formData.title} onChange={e => setFormData({ ...formData, title: e.target.value })} className="sm:col-span-2 w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required />
                      ...
                      <div className="w-full flex items-center gap-2">
                        ...
                        {formData.authorImage
                          ? <img src={formData.authorImage} alt="Preview" className="h-[38px] w-[38px] object-cover rounded-full shadow-sm border border-slate-200 flex-shrink-0" />
                          : <div className="h-[38px] w-[38px] bg-slate-100 rounded-full flex items-center justify-center border border-slate-200 flex-shrink-0"><User className="h-4 w-4 text-slate-400" /></div>}
                      </div>

                      {/* Cover Image Upload */}
                      <div className="sm:col-span-2 w-full flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                        <label className={`flex-1 border-2 border-dashed ${isUploading ? 'border-blue-400 bg-blue-50' : 'border-slate-300 hover:bg-slate-50'} rounded-lg p-2 cursor-pointer transition-colors flex items-center justify-center gap-2 h-[42px]`}>
                          {isUploading
                            ...
                            : <><UploadCloud className="h-4 w-4 text-slate-400" /><span className="text-slate-600 font-medium text-sm">Upload Cover Image</span></>}
                          <input type="file" className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'image')} disabled={isUploading} />
                        </label>
                        {formData.image && <img src={formData.image} alt="Preview" className="hidden sm:block h-[42px] w-20 object-cover rounded shadow-sm border border-slate-200 flex-shrink-0" />}
                      ...
                      <textarea placeholder="Short Excerpt (shows on cards)" value={formData.excerpt} onChange={e => setFormData({ ...formData, excerpt: e.target.value })} className="sm:col-span-2 w-full px-4 py-2 border rounded-lg h-20 focus:ring-2 focus:ring-blue-500 outline-none" required />
                      <textarea placeholder="Full Content block" value={formData.content} onChange={e => setFormData({ ...formData, content: e.target.value })} className="sm:col-span-2 w-full px-4 py-2 border rounded-lg h-40 focus:ring-2 focus:ring-blue-500 outline-none" required />
                    </div>
                ...
                <div className="divide-y divide-slate-100 border border-slate-100 rounded-lg">
                  {posts.map(post => (
                    <div key={post.id} className="p-4 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 hover:bg-slate-50 transition-colors">
                      <div className="flex gap-4 items-start sm:items-center min-w-0">
                        <img src={post.image} alt="" className="w-16 h-12 object-cover rounded-md flex-shrink-0" />
          ...
          {activeTab === 'categories' && (
            <div className="p-4 sm:p-6">
              ...
              <form onSubmit={handleAddCategory} className="flex flex-col sm:flex-row gap-3 mb-8">
                <input type="text" placeholder="New Category Name (e.g. Serverless)" value={newCategory} onChange={e => setNewCategory(e.target.value)} className="flex-1 px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 outline-none sm:max-w-sm" required />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {categories.map(cat => (
                  <div key={cat.id} className="flex items-center justify-between p-4 border border-slate-200 rounded-lg bg-slate-50 hover:border-slate-300 transition-colors shadow-sm">
                    ...
                    <button onClick={() => handleDeleteCategory(cat.id)} className="text-slate-400 hover:text-red-600 p-1 bg-white rounded shadow-sm border border-slate-200 flex-shrink-0"><Trash2 className="h-4 w-4" /></button>
                  </div>
                ))}
                {categories.length === 0 && <p className="text-slate-500 text-sm col-span-full">No categories yet. Add one above.</p>}
              </div>
            </div>
          )}
          ...
          {activeTab === 'contacts' && (
            <div className="flex flex-col md:flex-row min-h-[500px]">
              ...
              <div className={`${selectedMessage ? 'hidden md:flex' : 'flex'} flex-col w-full md:w-1/3 border-b md:border-b-0 md:border-r border-slate-200`}>
                <div className="p-4 border-b border-slate-200 bg-slate-50">
                  <h2 className="font-bold text-slate-800">Inbox</h2>
                </div>
                <div className="overflow-y-auto flex-1 md:h-[550px]">
                  {messages.map(msg => (
                    <button
                      key={msg.id}
                    ...
                    >
                      <div className="flex justify-between items-start mb-1">
                        ...
                        {msg.status === 'new' && <span className="w-2 h-2 rounded-full bg-blue-600 flex-shrink-0 mt-1"></span>}
                      </div>
              ...
              {/* Detail view — hidden on mobile when no message selected */}
              <div className={`${selectedMessage ? 'flex' : 'hidden md:flex'} flex-col flex-1 bg-slate-50/50`}>
                {selectedMessage ? (
                  <div className="p-4 sm:p-8">
                    {/* Back button — mobile only */}
                    <button
                      onClick={() => setSelectedMessage(null)}
                      className="md:hidden flex items-center gap-2 text-sm text-blue-600 font-medium mb-4"
                    >
                      ← Back to Inbox
                    </button>
                    ...
                    <div className="flex justify-between items-start mb-6 gap-4">
                      <div className="min-w-0">
                        <h3 className="text-xl font-bold text-slate-800 truncate">{selectedMessage.name}</h3>
                        <a href={`mailto:${selectedMessage.email}`} className="text-sm text-blue-600 hover:underline truncate block">{selectedMessage.email}</a>
                      </div>
                      <button onClick={() => handleDeleteMessage(selectedMessage.id)} className="text-red-500 hover:bg-red-50 p-2 rounded flex items-center text-sm flex-shrink-0">
                        <Trash2 className="h-4 w-4 mr-1" /> Delete
                      </button>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 mb-8 bg-white p-4 rounded-lg border border-slate-200 text-sm">
                    ...
                    <div>
                      ...
                      <div className="bg-white p-4 sm:p-6 rounded-lg border border-slate-200 text-slate-700 whitespace-pre-wrap">
